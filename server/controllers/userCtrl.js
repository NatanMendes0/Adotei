...
const editUser = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      return res
        .status(404)
        .json({ message: "Não é possível editar. Usuário não encontrado!" });
    }
...
const deleteUser = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      return res
        .status(404)
        .json({ message: "Não é possível excluir. Usuário não encontrado!" });
    }
...
const resetPassword = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    return res
      .status(404)
      .json({ message: "Não foi possível atualizar a senha!" });
  }
});
